...
/**
 * Comprehensive permission checking utility
 */
export class PermissionManager {
  ...
  static async hasPermission(
    ...
    // Get member with all permission data
    const member = await this.getMemberWithPermissions(memberId);
    if (!member) {
      return { granted: false, reason: 'DENIED' };
    }

    // Temporary fallback: Check old MemberRole system for admins
    const memberWithRole = await db.member.findUnique({
      where: { id: memberId },
      select: { role: true }
    });

    if (memberWithRole?.role === 'ADMIN') {
      return { granted: true, reason: 'LEGACY_ADMIN' };
    }

    // Check for administrator permission first (overrides everything)
    if (this.hasAdministratorPermission(member)) {
      return { granted: true, reason: 'ADMINISTRATOR' };
    }

    // Check user-specific overrides first (highest priority)
    const userOverride = this.getUserPermissionOverride(member, permission, scope, targetId);
    if (userOverride) {
      if (userOverride.grant === PermissionGrantType.ALLOW) {
        return { granted: true, reason: 'USER_OVERRIDE' };
      } else if (userOverride.grant === PermissionGrantType.DENY) {
